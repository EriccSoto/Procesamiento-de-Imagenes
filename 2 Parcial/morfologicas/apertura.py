import cv2
import numpy as np
#kernel mayor apeetura agresiva
...
def apertura_manual(imagen_binaria, tamaño_kernel=(3, 3)):
    # Erosión manual
    erosionada = erosion_manual(imagen_binaria, tamaño_kernel)
    cv2.imshow('Original binaria', imagen_binaria)
    cv2.imshow('paso 1 erosionada', erosionada)
    

    # Dilatación manual sobre la imagen erosionada
    dilatada = dilatacion_manual(erosionada, tamaño_kernel)
    cv2.imshow('paso 2 dilatada', dilatada)
   
    return dilatada

def apertura_con_opencv(imagen_binaria, tamaño_kernel=(5, 5)):
    # Librería
...
if __name__ == "__main__":
    ...
    apertura_manual = apertura_manual(imagen_binaria, (7, 7))
    apertura_opencv = apertura_con_opencv(imagen_binaria, (2, 2))

    cv2.imshow('Apertura manual', apertura_manual)

    cv2.waitKey(0)
    cv2.destroyAllWindows()
    cv2.imshow('Apertura con OpenCV', apertura_opencv)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

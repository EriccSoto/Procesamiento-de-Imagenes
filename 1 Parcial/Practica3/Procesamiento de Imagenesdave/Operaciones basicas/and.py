...
imagenAnd = np.zeros((int(alto02), int(ancho02), ncolores02),dtype='uint8')
for y in range(alto01):
    for x in range(ancho01):
        if (y + y_T) < alto02 and (x + x_T) < ancho01:
            imagenAnd[y,x,:] = imagen01[y,x,:] & imagen02[y,x,:]
 
cv2.imshow('Imagen de salida', imagenAnd)
cv2.waitKey(0)
